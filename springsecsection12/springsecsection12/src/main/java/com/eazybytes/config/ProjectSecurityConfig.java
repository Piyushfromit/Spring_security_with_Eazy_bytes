    ...
    SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.sessionManagement(sessionConfig -> sessionConfig.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                ...
                .csrf(csrfConfigurer -> csrfConfigurer.csrfTokenRequestHandler(csrfTokenRequestAttributeHandler)
                        .ignoringRequestMatchers("/contact", "/register", "/apiLogin")  // to ignore the CSRF Protection for these URLs
               ...
               // .sessionManagement(sessionManagementConfiguration ->  sessionManagementConfiguration.invalidSessionUrl("/invalidSession").maximumSessions(10).maxSessionsPreventsLogin(true))
                ...
                .authorizeHttpRequests((requests) -> requests
                        ...
                        .requestMatchers( "/myLoans").authenticated()   // Method laval security Using @PreAuthorize @PostAuthorize applied in controller and repository
                        .requestMatchers(  "/myCards").hasRole("USER")
                        .requestMatchers( "/user").authenticated()
                .requestMatchers( "/notices", "/contact", "/error", "/logout", "/register", "/invalidSession", "/apiLogin").permitAll());
